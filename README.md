...
recisdb and b25-sys are more convenient Rust wrappers for libaribb25. recisdb can read both Unix character device-based and BonDriver-based TV sources. 
...
- recisdb: reads a bitstream from both character devices and BonDriver
...
cargo build -p recisdb-cli -F dvb --release

# インストール
sudo cp -a target/release/recisdb /usr/local/bin

# または、cargo install でインストール
cargo install -F dvb --path recisdb-cli
```
